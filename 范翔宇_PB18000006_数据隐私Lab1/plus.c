...
void Anonymize(int arra[33000], int arrb[33000], char arrc[33000][30], int start, int end, int flag){
    //arra对应Age，arrb对应Gender，arrc对应occupation
    ...
    if(end - strict + 1< K || strict - start < K){
        ...
        if(end - strict + 1< K || strict - start < K){
            ...
            if(flag == 0){//之前是1，现在是0
                ...
                betens = max / 10;
                //泛化
                //两个维度都泛化为[最小-最大]，但是由于是字符，要分别挑出十位数、个位数单独标识
                for(i = start;i <= end;i ++){
                    CAge[writeindex][0] = '[';
                    ...
                    CAge[writeindex][6] = ']';
                    CGender[writeindex][0] = '[';
                    CGender[writeindex][1] = bstens + '0';
                    CGender[writeindex][2] = (min - bstens * 10) + '0';
                    CGender[writeindex][3] = '-';
                    CGender[writeindex][4] = betens + '0';
                    CGender[writeindex][5] = (max - betens * 10) + '0';
                    CGender[writeindex][6] = ']';
                    writeindex ++;
                }
            }else{
                //之前flag是0，现在是1
                ...
                for(i = start;i <= end;i ++){
                    ...
                    CGender[writeindex][0] = '[';
                    CGender[writeindex][1] = bstens + '0';
                    CGender[writeindex][2] = (arrb[start] - bstens * 10) + '0';
                    CGender[writeindex][3] = '-';
                    CGender[writeindex][4] = betens + '0';
                    CGender[writeindex][5] = (arrb[end] - betens * 10) + '0';
                    CGender[writeindex][6] = ']';
                    writeindex ++;
                }
            }
...
int main(){
    ...
    Scan();
    for(i = 0;i < MaxRow;i ++){
        if(CGender[i][0] != 'F' && CGender[i][0] != 'M'){
            printf("%s\n",CGender[i]);
        }
        if(CGender[i][0] == 'M'){
            Gender[i] = 0;
    ...
    //先对Age排序
    QuickSort(Age, Gender, occupation, 0, MaxRow - 1);
    //默认初始sel = 0；
    Anonymize(Age, Gender, occupation, 0, MaxRow - 1, 0);   
    for(i = 0;i < MaxRow;i ++){
        if(CGender[i][2] == '0' && CGender[i][5] == '0'){
            strcpy(CGender[i],"Male");
        }
        else if(CGender[i][2] == '1' && CGender[i][5] == '1'){
            strcpy(CGender[i],"Female");
        }else if(CGender[i][2] == '0' && CGender[i][5] == '1'){
            strcpy(CGender[i],"*");
        }
    }
    FILE *fpWrite=fopen("G:\\360MoveData\\Users\\Lenovo\\Desktop\\result_mon.txt","w");
    for(i = 0;i < MaxRow;i ++){
        fprintf(fpWrite, "%s\t%s\t%s\t\n",CAge[i],CGender[i],occupation[i]);
    }
    end = clock();
